...
<template>
  <div class="container">
    <PGTable :data="roles" title="角色列表" :showSelect="true" :total="total" v-model:page="page">
      <template v-slot:action>
        <auth-button auth="button/role_add" @click="onAddRole">添加</auth-button>
      </template>
      <template v-slot:content>
        <el-table-column align="center" prop="id" label="角色ID" sortable> </el-table-column>
        <el-table-column align="center" prop="role" label="角色名"> </el-table-column>
        <el-table-column align="center" prop="description" label="描述"> </el-table-column>
        <el-table-column align="center" label="权限">
          <template #default="scope">
            <el-button size="small" @click="handleAuth(scope.row, '权限详情')">查看</el-button>
            <auth-button auth="button/role_modify" size="small" @click="handleAuth(scope.row, '编辑权限')"
              >变更</auth-button
            >
          </template>
        </el-table-column>
