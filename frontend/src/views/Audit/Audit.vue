...
<template>
  <div class="container">
    <PGTable :data="logs" title="审计日志" :showSelect="false" :total="total" :onPageChanged="onPageChanged"
      :isExpand="true" v-model:expandData="expandLog">
      <template v-slot:content>
        <el-table-column type="expand">
          <div style="width: 100%; padding-left:10%;">
            <el-table height="200" :data="logChildrens"
        ...
        <el-table-column align="center" label="进度" style="display: flex;align-items: center;">
          <template #default="scope">
            <el-progress v-if=!scope.row.Isempty style="width: 100%" type="line"
              :percentage="scope.row.status == 'OK' ? 100 : 0">
              {{ scope.row.status == 'OK' ? '1/1' : '0/1' }}
            </el-progress>
            <el-progress v-else style="width: 100%" type="line"
              :percentage="(scope.row.status.split(',')[2] === '1.00' || scope.row.status.split(',')[2] === '0.00') ? 100 : scope.row.status.split(',')[2] * 100 || 0"
              :status="scope.row.status.split(',')[2] === '0.00' ? 'exception' : scope.row.status.split(',')[2] === '1.00' ? 'success' : 'warning'">
