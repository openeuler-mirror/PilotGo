...
function renameNode(node: any, data: any) {
  ElMessageBox.prompt("请输入节点名字", "编辑部门", {
    ...
    .then(({ value }) => {
      updateDepartment({ DepartID: data.id, DepartName: value })
        .then((resp: any) => {
          if (resp.code === 200) {
...
function deleteNode(node: any, data: any) {
  ElMessageBox.confirm("确定删除该部门？", "提示", {
    ...
    .then(() => {
      deleteDepartment({ DepartID: data.id, departName: data.label })
        .then((resp: any) => {
          if (resp.code === 200) {
...
function addNode(node: any, data: any) {
  ElMessageBox.prompt("请输入节点名字", "添加部门", {
    ...
    .then(({ value }) => {
      addDepartment({ PID: data.id, Depart: value })
        .then((resp: any) => {
          if (resp.code === 200) {
